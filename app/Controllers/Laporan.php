...
class Laporan extends BaseController {
    ...
    public function cetak() {
        ...
        if(!$awal || !$akhir) return "Pilih tanggal dulu!";

        // Simpan Log
        $this->db->table('cetak_log')->insert([
            'nama_pencetak' => session()->get('username'),
            'tgl_awal' => $awal, 'tgl_akhir' => $akhir, 'tgl_cetak' => date('Y-m-d H:i:s')
        ]);
        ...
        $data = $this->db->table('penjualan_detail d')
            ...
            ->select('m.no_invoice, m.tgl_keluar, s.nama_barang, d.qty_jual, d.total_harga, m.added_by')
            ->get()->getResultArray();

        return view('admin/cetak_pdf', ['data' => $data, 'awal' => $awal, 'akhir' => $akhir]);
    }
}